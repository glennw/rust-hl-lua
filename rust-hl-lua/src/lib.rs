...
impl Error for LuaError {
    fn description(&self) -> &str {
        match self {
            SyntaxError(_) => "Syntax error when parsing Lua code",
            ExecutionError(_) => "Error while executing Lua code",
            ReadError(_) => "Error while reading the Lua source code",
            WrongType => "Wrong type of data requested when executing Lua code",
        }
    }

    fn detail(&self) -> Option<String> {
        match self {
            SyntaxError(ref s) => Some(s.clone()),
            ExecutionError(ref s) => Some(s.clone()),
            _ => None,
        }
    }
    ...
    fn cause(&self) -> Option<&Error> {
        match self {
            ReadError(ref e) => Some(e as &Error),
            _ => None,
        }
    }
}
